...
module Description : Defs.ClassDescription = struct
  ...
  let runtimename = "Dom_type"
  let default_module = None
  let alpha = None
  let allow_private = false
  let predefs = [
    ["int"], ["Dom_type";"int"];
    ["int32"], ["Dom_type";"int32"];
    ["Int32";"t"], ["Dom_type";"int32"];
    ["int64"], ["Dom_type";"int64"];
    ["Int64";"t"], ["Dom_type";"int64"];
    ["bool"], ["Dom_type";"bool"];
    ["float"], ["Dom_type";"float"];
    ["string"], ["Dom_type";"string"];
    ["list"], ["Dom_type";"list"];
    ["array"],["Dom_type";"array"];
    ["option"], ["Dom_type";"option"];
  ]
  ...
  inherit Generator.generator
    ...
    method record ?eq ctxt tname params constraints (fields : Pa_deriving_common.Type.field list) =
      let to_dom =
        ...
        <:str_item<
          value to_default ?v () =
            ...
            do {{
              Dom_type.node = div ~a:[ a_class [ "dom_ext_" ^ $`str:tname$; "dom_ext_record"]] (List.map (fun (d,_) -> d) dom_list);
              Dom_type.value_ = `Record (List.map (fun (_,v) -> v) dom_list);
              Dom_type.error = None ;
            }};
      ...
      let save =
        ...
        <:str_item@here<
          value save dom_ext =
          match dom_ext.Dom_type.value_ with
             [ `Record value_list ->
      ...
      let to_dom =
        ...
        <:str_item@here<
          value to_default ?v () =
            ...
            let dom_list = $Helpers.expr_list to_default$ in
            do {{
              Dom_type.node = div ~a:[a_class [ "dom_ext_tuple" ]] (List.map (fun d -> d.Dom_type.node) dom_list) ;
              Dom_type.value_ = `List dom_list ;
              Dom_type.error = None ;
            }};
    ...
    (*
       ...
       we are saving a value of type Html5.div_content_fun in Dom_type.value_.
    ...
    *)
         ...
         let to_dom binds match_select =
           <:expr@here<
             ...
             let rec sel = lazy (Raw.select ~a:[ a_onchange (fun _ -> do { on_change ()})  ] $Helpers.expr_list (List.rev options)$)
             and on_change () =
             ...
             do {
               ...
               {
                 Dom_type.node = div ~a:[ a_class ["dom_ext_sum"]] [
                     (sel :> Eliom_content_core.Html5.elt Html5_types.div_content_fun);
                     updatable_div ;
                   ] ;
                 Dom_type.value_ = `Select (sel,$Helpers.expr_list dom_values$);
                 Dom_type.error = None ;
               }
             }
               >>
  ...
  let save =
    ...
    let mcs =
      List.fold_left (
        fun acc t ->
          mc (no_expr acc) (with_expr acc) t
      ) [ <:match_case@here< _ -> raise Dom_type.Empty_value >> ] tags
    in
